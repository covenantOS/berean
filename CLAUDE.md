...
- Rooms: `/workspace` (the pane-based studio: readers, the Library browser, the Factbook, lexicon and topic guides as tabs; `/`, `/library`, `/lexicon/[id]`, `/topics/[work]/[id]`, `/library/entity/[id]`, `/memory`, and the `/read` pickers permanently redirect into it), `/read/[book]/[chapter]` (the reader; the citation scheme) (+ `/plans`, `/search`), `/pulpit`, `/chapel`, `/desk`, `/almanac`, `/study`, `/settings`, `/sources`
