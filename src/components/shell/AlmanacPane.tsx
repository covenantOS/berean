...
/** A calendar row: an explicit entry or an appointed sermon, one timeline. */
interface CalendarRow {
  key: string;
  date: string;
  title: string;
  series?: string;
  /** Opens the linked project or service tab; absent for an unlinked note. */
  open?: () => void;
  kind: string;
  removable: boolean;
}
...
/**
 * The Almanac pane: the preaching and teaching calendar with the rule of
 * life, moved from the retired /almanac page. Explicit entries and appointed
 * sermons share one date-sorted timeline; a row linked to a project or a
 * service opens its tab, and the day's appointed readings dispatch
 * berean:open-ref, carrying the pane in focus to the text. The rule of life
...
export default function AlmanacPane() {
  ...
  const timeline: CalendarRow[] = [
    ...entries.map((e) => ({
      ...
      series: e.series,
      open: e.projectId
        ? () => dispatch({ type: "openProject", projectId: e.projectId!, title: e.title })
        : e.liturgyId
          ? () => dispatch({ type: "openService", serviceId: e.liturgyId!, title: e.title })
          : undefined,
    ...
    ...sermons.map((p) => ({
      ...
      title: p.title,
      series: p.series,
      open: () => dispatch({ type: "openProject", projectId: p.id, title: p.title }),
      kind: "sermon",
      removable: false,
    })),
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <header className="border-b border-rule pb-2">
        <p className="small-caps text-xs font-semibold text-amber">The Almanac</p>
        <h2 className="mt-0.5 font-editorial text-lg font-semibold">The room that governs time</h2>
        <p className="no-print mt-0.5 text-[0.68rem] text-muted">
          The preaching and teaching calendar, the rule of life, and the day&apos;s appointed work ·
          everything done decently and in order
        </p>
        <p className="no-print mt-0.5 text-[0.68rem] text-muted">
          For the long view of the time God has governed, see{" "}
          <button
            type="button"
            onClick={() => dispatch({ type: "openTimeline" })}
      ...
      <section className="mt-6 mb-10 rounded-[4px] border border-rule bg-surface p-5">
        ...
        <ul className="space-y-2 text-sm">
          ...
          {dueMemory.length > 0 && (
            <li>
              ...
              <button
                type="button"
                onClick={() => dispatch({ type: "openMemory", passageId: dueMemory[0].id })}
                className="text-sapphire hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-sapphire"
              >
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <section>
          <h3 className="small-caps mb-2 text-sm text-muted">Preaching &amp; teaching calendar</h3>
        ...
        {/* Rule of life */}
        <section>
          <h3 className="small-caps mb-2 text-sm text-muted">The rule of life</h3>
          ...
          <div>
            ...
            {t.open ? (
              <button
                ...
                onClick={t.open}
                title={`Open ${t.title}`}
                className="font-medium text-ink hover:text-sapphire focus-visible:outline focus-visible:outline-2 focus-visible:outline-sapphire"
              >
                {t.title}
              </button>
            ) : (
