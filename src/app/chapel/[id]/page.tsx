...
export default function LiturgyPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const liturgy = useRecord(liturgies, id);
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
        <p className="text-sm text-muted">
          ...
          <Link href="/chapel" className="text-sapphire">
            Back to the Chapel
          </Link>
        </p>
      </div>
  ...
  const setElements = (elements: LiturgyElement[]) => liturgies.update(id, { elements });
  ...
  async function askScribe() {
    ...
    try {
      ...
      liturgies.update(id, { elements, rationale });
      setScribeState("idle");
    } catch (err) {
      setScribeState("error");
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      <nav className="mb-6 text-sm text-muted no-print">
        <Link href="/chapel" className="text-sapphire no-underline hover:underline">
          The Chapel
        </Link>{" "}
        / {liturgy.title}
      </nav>
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3 no-print">
        <div>
          <input
            value={liturgy.title}
            onChange={(e) => liturgies.update(id, { title: e.target.value })}
          ...
          <input
            type="date"
            value={liturgy.date ?? ""}
            onChange={(e) => liturgies.update(id, { date: e.target.value || undefined })}
