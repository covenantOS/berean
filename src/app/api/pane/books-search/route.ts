 ...
 * shelf's sections, the Spurgeon sermon archive, and the topical works'
 * entries server-side (src/lib/booksearch.ts), each group capped at
 * HIT_CAP with its full total beside it so a truncated group says so. The
 * field parameter scopes the query to headings or body text where the data
 * carries the distinction; anything else reads as "all". A malformed
 * precise query answers with an error message instead of hits, and the
 * pane shows it.
 */
