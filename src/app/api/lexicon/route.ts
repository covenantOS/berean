...
/**
 * Whole-dictionary search: ids, lemmas, transliterations, and definitions
 * across both Strong's dictionaries, capped at sixty rows per dictionary.
 * Matching folds diacritics, so "agape" finds "agapē". This is the retired
 * /lexicon index's query moved behind a route so the workspace's lexicon
    ...
    try {
      ...
      for (const [id, entry] of Object.entries(dict)) {
        if (
          ...
          entry.tyndale?.some((v) => fold(v.gloss).includes(needle) || fold(v.def).includes(needle))
        ) {
