...
- [x] [SW] Print and export for notes and clippings. (Notes print from the Documents rail's Notes section through the shared print idiom (data-print-root + PrintButton): the current filter's notes (the whole list, one notebook, or the passage in focus) render grouped by notebook with references, full text, and dates, while the rail's interactive chrome steps aside. The existing exportNotesMarkdown helper (src/lib/marginalia.ts) is wired to an Export download beside it, and the journal page prints the same way. Clippings print the same way from the listdoc pane, excerpts with their citations and the interactive chrome stepped aside; Copy all excerpts stands as their export, one clipboard write in the device's citation style.)
- [x] [SW] Journaling: date-based notebooks and filters used as a journal. (A journal entry is a note carrying a date (date on the marginalia record, distinct from the verse anchor; a note may carry an anchor, a date, or both), so old notes load unchanged and every entry answers in Docs Search without a new collection. The /journal page (the /memory, /prayers precedent) captures against a day, today by default, edits and deletes in place, reads chronologically grouped by day with references where an anchor exists, and prints; the Documents rail's Notes section links to it.)
