...
- [x] [SW] Automatic citations: copied text carries source citations in the style set in Program Settings. (Every copy path formats through formatCitation (src/lib/citation.ts), which reads the persisted style (berean.citationStyle.v1) the workspace Settings rail writes: the verse context menu and the selection toolbar (ReaderMenus.tsx), the context strip (ReaderPane.tsx), a passage list's copy-all (ListDocPane.tsx), and Power Lookup (src/lib/powerLookup.ts). Text-first and citation-first are the two styles.)
